...
local function RegisterPostGameMode()
    ...
    _G.EQUIP_TRAITOR_BYPASS = GenerateNewEquipmentID()
    local TraitorBypass = {
    ...
    if SERVER then
        ...
        if TTT2 then
            timer.Create(HOOK_ID .. "Timer", 5, 1, function ()
                net.Start(NET_MSG_MESSAGE)
                net.WriteString("[TTTKMENZMOD] Loaded TTT2")
                net.Broadcast()
            end)
        else
            timer.Create(HOOK_ID .. "Timer", 5, 100, function ()
                net.Start(NET_MSG_MESSAGE)
                net.WriteString("[TTTKMENZMOD] Loaded TTT1")
                net.Broadcast()
            end)
